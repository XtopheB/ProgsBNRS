...
% 04/10/2011 : Ajout des indices de Malmquists

% La commande est simplement :  Sweave("MonSweave1.Rnw")
\documentclass[a4paper]{article}

\title{Tests sur Secteur 151C\\ Fichier NON cylindré \& ``atypiques exclus'' \\ 
Fimres de référence aléatoirement sélectionnées}
\author{Christophe }
...
# ici on tire aaléatoirement les données
#set.seed(12345)
#set.seed(23456)
set.seed(45678)
nbsample <- 200
...
% ------------Debut du papier -------------
\bigskip 
On a \textbf{\Sexpr{nrow(dataall)}} observations (toutes années confondues) et on a un échantillon de 
\textbf{\Sexpr{nrow(dataobs)}} firmes pour lesquelles l'effciacité est mesurée.
...
%% AJOUT Des calculs des Malmquists issu de TestPapierBench2.Rnw   04/10/2011
<<echo=FALSE, results=hide>>=
library(FEAR)
library(xtable)
# Données Malmquist individuelles sur 1996-2000 (25/03/2011)

data1 <- subset(dataall,an1x==1996)  # <-  année 1
n1=nrow(data1)
...
data1.id=t(matrix(c(data1$ident),nrow=n1,ncol=1))
 ...
 data2 <- subset(dataall,an1x==2000)  # <-  année 2
n2=nrow(data2)
...
foo <- data.frame(malm.all)
write.dta(foo, "Malmquist1996-2000.dta")
@


<<echo=FALSE, results=hide>>=
# Données Malmquist individuelles sur 2000-2006 (23/05/2011)

data1 <- subset(dataall,an1x==2000)  # <-  année 1
n1=nrow(data1)
data1.x=t(matrix(c(data1$K,data1$L,data1$M),nrow=n1,ncol=3))
data1.y=t(matrix(c(data1$Y),nrow=n1,ncol=1))
data1.id=t(matrix(c(data1$ident),nrow=n1,ncol=1))

 data2 <- subset(dataall,an1x==2006)  # <-  année 2
n2=nrow(data2)
data2.x=t(matrix(c(data2$K,data2$L,data2$M),nrow=n2,ncol=3))
data2.y=t(matrix(c(data2$Y),nrow=n2,ncol=1))
data2.id=t(matrix(c(data2$ident),nrow=n2,ncol=1))
...
malm.all <-malmquist(LIST=malm.com)
foo <- data.frame(malm.all)
write.dta(foo, "Malmquist2000-2006.dta")
@

<<echo=FALSE, results=hide>>=
# Données Malmquist individuelles sur 1996-2006 (04/10/2011)
...
malm.all <-malmquist(LIST=malm.com)
foo <- data.frame(malm.all)
write.dta(foo, "Malmquist1996-2006.dta")
@
